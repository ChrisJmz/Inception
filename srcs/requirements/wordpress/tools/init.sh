...
if  [ ! -f "$PROTECT_FILE" ]; then
	...
	wp core install --url="localhost" --title="INCEPTION" --admin_user=$WP_ADMIN_USER --admin_password=$WP_ADMIN_PASSWORD --admin_email=$WP_ADMIN_EMAIL --path="/var/www/html/wordpress/" --allow-root
